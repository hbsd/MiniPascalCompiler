...
public enum PascalParser implements IParser
{
     ...
     * 
     * @Override protected IParser rule() { return
     * seq(opt(SIGN),SIMPLE_EXPRESSION); }
     * 
     * @Override protected INode success(INode node) { final SequenceNode
     * n=(SequenceNode)node; if(n.get(0) instanceof EmptyNode){ return new
     * SignedSimpleExpression((ISimpleExpression)n.get(1)); }else{ return new
     * SignedSimpleExpression((SignLiteral)n.get(0),
    ...
    protected INode failure(INode node) {
        ...
        {
            return new FailureNode(node, msg);
        }
        else
        {
            assert false;
